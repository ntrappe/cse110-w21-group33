...
describe('Test Timer Button UI', { includeShadowDom: true }, () => {
  ...
  it('Check button colors when hovering over', () => {
    cy.get('#timer-button').trigger('mouseover');
    cy.get('#timer-button').should('have.css', 'background-color', 'rgb(49, 54, 60)');
    cy.get('#timer-button').should('have.css', 'border-color', 'rgb(141, 148, 157)');
  });
});