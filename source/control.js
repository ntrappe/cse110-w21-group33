...
const resizeElements = () => {
  ...
  if (window.innerHeight < MIN_HEIGHT) {
    inputText = inputText.concat('translateY(')
        .concat((window.innerHeight < MIN_HEIGHT_STOP_CHANGE ? -100 
        : (window.innerHeight - MIN_HEIGHT) / 2).toString()).concat('px)');
  }

  if (window.innerWidth < MIN_WIDTH) {
    inputText = inputText.concat(' scale(')
        .concat((window.innerWidth / MIN_WIDTH).toString()).concat(') ');
  }
  ...
  if (window.innerWidth < MIN_WIDTH) {
    pomoFinish.changeTransform('scale('
        .concat((window.innerWidth / MIN_WIDTH).toString()).concat(') translateX(')
        .concat(((MIN_WIDTH - window.innerWidth) / 
        window.innerWidth * MIN_WIDTH / X_OFFSET_FACTOR).toString()).concat('px)'));
    pomoInfo.changeTransform('scale('
        .concat((window.innerWidth / MIN_WIDTH).toString()).concat(') translateX(')
        .concat(((MIN_WIDTH - window.innerWidth) / 
        window.innerWidth * MIN_WIDTH / X_OFFSET_FACTOR).toString()).concat('px)'));
    pomoSettings.changeTransform('scale('
        .concat((window.innerWidth / MIN_WIDTH).toString()).concat(') translateX(').concat(  
        ((window.innerWidth - MIN_WIDTH ) / 
        window.innerWidth * MIN_WIDTH / X_OFFSET_FACTOR).toString()).concat('px)'), 
        'scale('.concat( (window.innerWidth / MIN_WIDTH).toString()).concat(') translateX(')
        .concat(((window.innerWidth - MIN_WIDTH ) / 
        window.innerWidth * MIN_WIDTH / X_OFFSET_FACTOR).toString()).concat('px)'),
